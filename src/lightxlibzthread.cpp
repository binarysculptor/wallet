...
#include "lightxlibzthread.h"
#include "main.h"

/****** Thread ********/
void CLightWorker::ThreadLightXlibzSimplified()
{
    RenameThread("liberty-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "liberty-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                try {
                    ...
                    LogPrintStr(std::string("ThreadLightXlibzSimplified: ") + e.message + "\n");
                ...
                if (!res) {
                    ...
                    ss << heightStop;
                    if (genWit.getPfrom()) {
                        LogPrintf("%s pushing message to %s \n", "liberty-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightxlibzthread");
            break;
        }
    }
}
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber)
{
    if (wit.getStartingHeight() == blockHeight) {
        LogPrintf("%s rejecting work %s , error code: %s\n", "liberty-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
