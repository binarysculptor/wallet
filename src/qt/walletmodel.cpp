...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 LBRT.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzLBRTReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzLBRTReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzLBRTReset.connect(boost::bind(NotifyzLBRTReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzLBRTReset.disconnect(boost::bind(NotifyzLBRTReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zlbrtTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
