...
#include "ui_zlbrtcontroldialog.h"
...
std::set<std::string> ZLbrtControlDialog::setSelectedMints;
std::set<CMintMeta> ZLbrtControlDialog::setMints;

ZLbrtControlDialog::ZLbrtControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZLbrtControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZLbrtControlDialog::~ZLbrtControlDialog()
{
    delete ui;
}

void ZLbrtControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLbrtControlDialog::updateList()
{
...
void ZLbrtControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLbrtControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZLbrt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLbrtControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZLbrtControlDialog::GetSelectedMints()
{
...
void ZLbrtControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
