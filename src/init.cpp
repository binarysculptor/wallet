...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the Liberty and XLBz money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-bantime=<n>", strprintf(_("Number of seconds to keep misbehaving peers from reconnecting (default: %u)"), 14400));
    ...
    strUsage += HelpMessageOpt("-xlbzstake=<n>", strprintf(_("Enable or disable staking functionality for XLBz inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupxlbz=<n>", strprintf(_("Enable automatic wallet backups triggered after each XLBz minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-xlbzbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic XLBz backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 10416, 20416));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CXLBzWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize XLBzWallet
        uiInterface.InitMessage(_("Syncing XLBz wallet..."));

        bool fEnableXLBzBackups = GetBoolArg("-backupxlbz", true);
        pwalletMain->setXLBzAutoBackups(fEnableXLBzBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->xlbzTracker->Init();
