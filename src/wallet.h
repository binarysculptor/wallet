...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LBRT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zLBRT send
enum ZerocoinSpendStatus {
    ZLBRT_SPEND_OKAY = 0,                            // No error
    ZLBRT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLBRT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLBRT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLBRT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLBRT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLBRT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLBRT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLBRT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLBRT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLBRT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLBRT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLBRT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLBRT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLBRT_SPENT_USED_ZLBRT = 14,                      // Coin has already been spend
    ZLBRT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLBRT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZLbrtBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZLBRTOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    void setZLbrtAutoBackups(bool fEnabled)
    {
    ...
    /** zLBRT reset */
    boost::signals2::signal<void()> NotifyzLBRTReset;
