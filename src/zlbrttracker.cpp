...
CzLBRTTracker::CzLBRTTracker(std::string strWalletFile)
{
...
CzLBRTTracker::~CzLBRTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLBRTTracker::Init()
{
...
bool CzLBRTTracker::Archive(CMintMeta& meta)
{
...
bool CzLBRTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLBRTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLBRTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLBRTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLBRTTracker::GetSerialHashes()
{
...
CAmount CzLBRTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLBRTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLBRTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLBRTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLBRTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLBRTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLBRTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLBRTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLBRTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLBRTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLBRTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLBRTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLBRTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLBRTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLBRTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLBRTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLBRTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
