...
#ifndef LIBERTY_ZLBRTWALLET_H
#define LIBERTY_ZLBRTWALLET_H
...
public:
    CzLbrtWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZLBRT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZLBRT(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //LIBERTY_ZLBRTWALLET_H
