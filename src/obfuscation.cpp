...
/* *** BEGIN OBFUSCATION MAGIC - Liberty **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Liberty and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 Liberty per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeLibertyAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100Liberty+1 ( bit on if present )
    // bit 1 - 10Liberty+1
    // bit 2 - 1Liberty+1
    // bit 3 - .1Liberty+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsValidMasternode(CTxIn& vin, CPubKey& pubkey)
{
    CScript payee2;
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
            if (CMasternode::IsMasternodeCollateral(out.nValue)) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("liberty-obfuscation");
