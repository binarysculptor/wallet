...
public:
    ...
    bool WriteZLBRTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLBRTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLBRTSeed_deprecated(uint256& seed);
    bool EraseZLBRTSeed();
    bool EraseZLBRTSeed_deprecated();

    bool WriteZLBRTCount(const uint32_t& nCount);
    bool ReadZLBRTCount(uint32_t& nCount);
