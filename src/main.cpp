            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-xlbz");
            }
        } else {
            LOCK(pool.cs);
...
bool RecalculateXLBSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZXLBSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new XLBz supply for block=%s height=%d", __func__,
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nVersion < 5 && nHeight > 2500 && CBlockIndex::IsSuperMajority(5, pindexPrev, Params().RejectBlockOutdatedMajority())) {
...
{
    if (CZPivStake* XLBz = dynamic_cast<CZPivStake*>(stake)) {
