...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 299 && nHeight >= 150) {
        nSubsidy = 30 * COIN;
    } else if (nHeight <= 449 && nHeight >= 300) {
        nSubsidy = 40 * COIN;
    } else if (nHeight <= 7199 && nHeight >= 450) {
        nSubsidy = 50 * COIN;
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CXlibzStake* XLIBz = dynamic_cast<CXlibzStake*>(stake)) {
        ...
        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: XLIBz stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> xlibInputs;
        std::vector<CTxIn> xlibzInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if (stakeIn.scriptSig.IsZerocoinSpend()) {
                xlibzInputs.push_back(stakeIn);
            } else {
                xlibInputs.push_back(stakeIn);
            }
        }
        const bool hasXlibInputs = !xlibInputs.empty();
        const bool hasXlibzInputs = !xlibzInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in : tx.vin) {
                ...
                if (hasXlibInputs)
                    ...
                    for (const CTxIn& xlibIn : xlibInputs) {
                        if (xlibIn.prevout == in.prevout) {
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in : t.vin) {
                        ...
                        for (const CTxIn& stakeIn : xlibInputs) {
            ...
            // Now that this loop if completed. Check if we have XLIBz inputs.
            if (hasXlibzInputs) {
                for (const CTxIn& xlibzInput : xlibzInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(xlibzInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxlib");
            ...
            if (!isBlockFromFork)
                for (const CTxIn& xlibzInput : xlibzInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpend(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100, error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxlib");
                }
        }
    }
        ...
        if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zxlib", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zxlib", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
