...
*Setup instructions for a gitian build of Liberty using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Liberty
...
- [Building Liberty](#building-liberty)
...
git clone https://github.com/liberty-crypto/liberty.git
cd liberty
...
To build Liberty (for Linux, OSX and Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/crowning-/liberty.git
